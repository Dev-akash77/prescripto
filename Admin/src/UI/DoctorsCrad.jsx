...
const DoctorsCrad = ({ data }) => {
  ...
  return (
    <div className="border border-[#abababdd] rounded-xl overflow-hidden flex flex-col cursor-pointer hover:border-blue duration-500">
      <div className="bg-blueTrans cc">
        <img src={data.image} alt={data.name} className="w-[14rem]" />
      </div>
      <div className="p-3 flex flex-col gap-1">
        <h2 className="font-semibold text-lg">{data.name}</h2>
        <p className="text-[#7d7d7ddd]">{data.speciality}</p>
        <div className="flex gap-1" onClick={()=>{changeDoctorAvailable(data)}}>
          {" "}
          <input
            type="checkbox"
