...
import doctor from "../Api/Doctor.json";
  ...
  return (
    <div>
      ...
      <div className="grid place-content-center md:grid-cols-5 grid-cols-1 gap-5 w-full mt-[3rem]">
        {doctor.slice(0, 10).map((cur, id) => {
          return <DoctorsCard key={id} data={cur} />;
        })}
      </div>
      <div className="cc mt-[4rem]">
        <NavLink to={"/doctors"} className="py-[.8rem] px-[3rem] rounded-full bg-blueTrans border border-blue">More</NavLink>
      </div>
    </div>
  );
};
